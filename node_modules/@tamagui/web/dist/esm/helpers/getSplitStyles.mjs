...
const getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, context, elementType, debug, skipThemeTokenResolution) => {
  ...
  for (const keyOg in props) {
    ...
    if (staticConfig.accept && (staticConfig.accept[keyInit] === "style" || staticConfig.accept[keyInit] === "textStyle") && typeof valInit == "object") {
      const styleObject = getSubStyle(styleState, keyInit, valInit, styleProps.noClassNames);
      viewProps[keyInit] = styleObject;
      continue;
    }
    if (process.env.NODE_ENV === "development" && debug === "verbose" && console.groupEnd(), styleProps.disableExpandShorthands || keyInit in shorthands && (keyInit = shorthands[keyInit]), keyInit === "className" || keyInit in usedKeys || props.asChild && webViewFlexCompatStyles[keyInit] === valInit) continue;
  ...
  if (!styleProps.noNormalize && !staticConfig.isReactNative && !staticConfig.isHOC && (!styleProps.isAnimated || conf.animations.supportsCSSVars) && Array.isArray(styleState.style?.transform) && (styleState.style.transform = transformsToString(styleState.style.transform)), styleState.style && shouldDoClasses) {
    ...
    if (!styleState.style.$$css) {
      ...
      for (const atomicStyle of atomic) {
        ...
        isAnimatedAndAnimateOnly ? (retainedStyles ||= {}, retainedStyles[key] = styleState.style[key]) : nonAnimatedAnimateOnly ? (retainedStyles ||= {}, retainedStyles[key] = atomicStyle.value, shouldRetain = !0) : (addStyleToInsertRules(rulesToInsert, atomicStyle), mergeClassName(transforms, classNames, key, atomicStyle.identifier, !1, !0));
      }
...
function addStyleToInsertRules(rulesToInsert, styleObject) {
  shouldInsertStyleRules(styleObject.identifier) && (process.env.TAMAGUI_REACT_19 || updateRules(styleObject.identifier, styleObject.rules), rulesToInsert.push(styleObject));
}
